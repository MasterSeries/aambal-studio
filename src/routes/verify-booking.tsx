...
export default function VerifyBookingPage() {
    ...
    return (
      <div className="min-h-screen bg-[#fcfaf9] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-semibold text-[#a3889f] tracking-widest uppercase">Fetching Details...</p>
        </div>
      </div>
    );
  }
  ...
  const statusColor = isApproved ? "emerald" : isRejected ? "red" : "amber";
  const statusText = isApproved ? "Valid & Active" : isRejected ? "Rejected / Invalid" : "Under Processing";
      ...
      >
        <div className="flex-1 flex flex-col relative items-center">
          <motion.div 
            ...
            className={`w-full max-w-md bg-[#fffaf5] rounded-[32px] p-6 shadow-[0_10px_40px_rgba(0,0,0,0.08)] border border-white relative z-20 ${!isApproved ? 'opacity-80 grayscale-[30%]' : ''}`}
          >
            ...
            <div className="border-t border-dashed border-slate-200 pt-6 px-4">
              <div className={`text-slate-800 mb-2 ${!isApproved ? 'opacity-30' : ''}`}><Icons.Barcode /></div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex-1 w-full flex items-center justify-center mt-12 relative">
             ...
             <div className="relative w-full max-w-[400px] aspect-square transform hover:scale-105 transition-transform duration-700 ease-out">
                ...
                <div className={`absolute top-[20%] left-1/2 -translate-x-1/2 w-[60%] h-[50%] bg-gradient-to-tr from-[#fcfaf9] to-white rounded-2xl transform rotate-[-30deg] skew-x-[30deg] shadow-[20px_20px_40px_rgba(0,0,0,0.15)] flex flex-col items-center justify-center p-6 border border-white/60`}>
                   ...
                   {isPending && (
                     <>
                       <div className="w-16 h-16 bg-amber-100 rounded-full flex items-center justify-center text-amber-500 mb-4 shadow-inner"><Icons.Clock /></div>
                       <p className="text-lg font-bold text-slate-800 text-center">Awaiting Admin</p>
                       <p className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold mt-1 text-center">Processing Request</p>
                     </>
                   )}
                   {isRejected && (
                     <>
                       <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center text-red-500 mb-4 shadow-inner"><Icons.Close /></div>
                       <p className="text-lg font-bold text-slate-800">Pass Voided</p>
                       <p className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold mt-1">Invalid or Cancelled</p>
                     </>
                   )}
        ...
        <div className="w-full xl:w-[350px] flex flex-col gap-6 relative z-20">
          <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.3 }} className={`bg-${statusColor}-500 rounded-[32px] p-6 text-white shadow-lg relative overflow-hidden`}>
            ...
            <div className="flex items-center gap-3 mb-6 relative z-10">
              <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm">
                {isApproved ? <Icons.CheckShield /> : isPending ? <Icons.Clock /> : <Icons.Close />}
              </div>
              <div>
                <p className="text-[11px] uppercase tracking-widest font-semibold opacity-90">System Status</p>
          ...
          <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="bg-[#fffdfb]/80 backdrop-blur-md rounded-[32px] p-6 border border-white/80 shadow-sm flex-1">
            ...
            <div className="space-y-5">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Primary Guest</p>
                  <p className="text-sm font-bold text-slate-800">{clientName}</p>
                  {partnerName && <p className="text-xs text-slate-500 font-medium mt-0.5">+ {partnerName}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-slate-100">
              <div className="bg-[#fcfaf9] border border-slate-100 rounded-2xl p-4 flex items-center justify-between">
                ...
                <span className={`bg-${statusColor}-50 text-${statusColor}-600 text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider`}>
                  {isApproved ? "Confirmed" : isPending ? "Pending" : "Cancelled"}
                </span>
              </div>
            </div>
          </motion.div>
