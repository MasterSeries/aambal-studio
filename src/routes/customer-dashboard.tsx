...
export default function CustomerDashboard() {
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // Listen to live updates for the specific user's photography bookings
        onSnapshot(qPhotos1, (snap) => {
           const liveData: any[] = [];
           snap.forEach(d => liveData.push({ id: d.id, type: "photo", ...d.data() }));
           liveData.sort((a,b) => (b.createdTimestamp?.toMillis?.() || 0) - (a.createdTimestamp?.toMillis?.() || 0));
           if(liveData.length > 0) setBookings(liveData);
        });
  ...
  // Combine items for the Timeline View
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[-1] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-white/80 to-white/30" />
      </div>

      <div className="min-h-screen p-4 sm:p-6 md:p-8 max-w-[1400px] mx-auto relative z-10">
        
        {/* ── HEADER ── */}
        <div className="flex justify-between items-center mb-8">
          <h1 style={{ fontFamily: 'DormyCustom', fontSize: '2.5rem', fontWeight: 800, letterSpacing: '-0.02em', color: '#111' }}>
            Dashboard
          </h1>
          <button onClick={handleLogout} className="bg-white/60 hover:bg-white text-black px-6 py-2.5 rounded-full font-bold text-sm transition-all border border-black/10 flex items-center gap-2 shadow-sm">
            <Icons.LogOut /> Logout
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          {/* COLUMN 2: Horizontal Schedule View (Center) */}
          <div className="lg:col-span-6 flex flex-col">
            <GlassPanel className="p-8 flex-1 bg-white/50 border-white/60 shadow-sm flex flex-col">
              <div className="flex justify-between items-end mb-8">
                ...
                <div className="text-4xl font-light text-gray-400">{timelineItems.length}<span className="text-sm text-gray-400 font-bold ml-1 uppercase tracking-widest">events</span></div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-6">
                {timelineItems.length === 0 ? (
                  ...
                  timelineItems.map((item, idx) => {
                    ...
                    const displayDate = itemDate?.includes("-") ? new Date(itemDate).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }) : itemDate;
                    
                    return (
                      <div key={idx} className="flex gap-4 group relative">
                        {/* Timeline Line */}
                        <div className="flex flex-col items-center">
                          <div className="w-3.5 h-3.5 rounded-full bg-gray-900 shadow-[0_0_0_4px_rgba(0,0,0,0.05)] relative z-10" />
                          {idx !== timelineItems.length - 1 && <div className="w-px h-full bg-gray-300 mt-2" />}
                        </div>
                        
                        {/* Event Content Pill (Black Horizontal Layout) */}
                        <div className="flex-1 pb-8">
                          <p className="text-[11px] font-bold text-gray-500 mb-2">{displayDate}</p>
                          <div className="bg-[#0a0a0c] text-white rounded-3xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center shadow-lg gap-4">
                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center border border-white/10 text-white/80">
                            ...
                            <div className="w-full sm:w-auto text-right">
                              <span className="bg-white/10 text-white border border-white/20 px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest inline-block">
                                {item.status || "Pending"}
                              </span>
                            </div>
                          </div>
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            ...
            <GlassPanel className="p-6 flex-1 bg-white/60 border-white/60 shadow-sm overflow-hidden flex flex-col min-h-[300px]">
                    ...
                    return (
                      <div key={b.id} className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => setViewQrBooking(b)} className="flex-1 bg-gray-50 hover:bg-gray-100 text-gray-900 py-3 rounded-2xl text-xs font-bold transition-colors border border-gray-100">
                            View QR
                          </button>
                        ...
                        {!isAppr && (
                          <div className="flex gap-2 mt-2">
                            <button onClick={() => openReschedule(b)} className="flex-1 bg-white text-gray-600 py-2 rounded-2xl text-[9px] font-bold uppercase transition-colors">Reschedule</button>
                            <button onClick={() => cancelBooking(b.id)} className="flex-1 bg-white text-red-500 hover:bg-red-50 py-2 rounded-2xl text-[9px] font-bold uppercase transition-colors">Cancel</button>
                          </div>
                        )}
                      </div>
                    )
